...
//----------------------------------------------------------------------------
PhongModels.Gouraud.vertexShader = `
    ...
    void main() {
        ...
        vec3 specular_term = Ip_diffuse_color * k_s * pow(dot(R_cam_spc, normal), n);
...
//----------------------------------------------------------------------------
PhongModels.Gouraud.fragmentShader = `
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-= Phong Vertex Shader =-=-=-=-=-=-=-=-=-=-=-=-=-=
//----------------------------------------------------------------------------
PhongModels.Phong.vertexShader = `
    varying mat4 modelViewM4;
    varying mat3 normalM3;
    varying vec3 normalVec3;
    varying vec3 positionVec3;
...
//----------------------------------------------------------------------------
PhongModels.Phong.fragmentShader = `
    varying mat4 modelViewM4;
    varying mat3 normalM3;
    varying vec3 normalVec3;
    varying vec3 positionVec3;
    ...
    void main() {
        // 'modelViewM4' : variável de sistema que contém a matriz ModelView (4x4).
        // 'Ip_pos_cam_spc' : variável que armazenará a posição da fonte de luz no Espaço da Câmera.
        ...
        vec4 P_cam_spc = modelViewM4 * vec4(positionVec3, 1.0);

        // 'normalVec3' : variável de sistema que contém o vetor normal do vértice (vec3) no espaço do objeto.
        // 'normalM3' : variável de sistema que contém a matriz de normais (3x3) gerada a partir da matriz 'modelViewMatrix'.
        
        vec3 N_cam_spc = normalize(normalM3 * normalVec3);
        ...
        vec3 ambient_term = Ia * k_a;
        vec3 diffuse_term = Ip_diffuse_color * k_d * max(0.0, dot(N_cam_spc, L_cam_spc));
        vec3 specular_term = Ip_diffuse_color * k_s * pow(dot(R_cam_spc, normalVec3), n);
...
let material = new THREE.ShaderMaterial({
  ...
  // Gouraud Shading
  // vertexShader: PhongModels.Gouraud.vertexShader,
  // fragmentShader: PhongModels.Gouraud.fragmentShader,

  // Phong Shading
  vertexShader: PhongModels.Phong.vertexShader,
  fragmentShader: PhongModels.Phong.fragmentShader
});
